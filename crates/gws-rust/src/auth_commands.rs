...
///
/// Use `gwsr auth login --full` to request these.  Unverified OAuth apps will
...
pub fn config_dir() -> PathBuf {
    if let Ok(dir) = std::env::var("GWSR_CONFIG_DIR") {
...
fn plain_credentials_path() -> PathBuf {
    if let Ok(path) = std::env::var("GWSR_CREDENTIALS_FILE") {
...
/// Build the clap Command for `gwsr auth`.
...
/// Build the clap Command for `gwsr auth login` (used by `run_login` for
...
async fn handle_login_inner(
    ...
    // 2. Saved client_secret.json from `gwsr auth setup` or manual download
    ...
    let output = json!({
        ...
        "encryption": "AES-256-GCM (key in OS keyring or local `.encryption_key`; set GWSR_KEYRING_BACKEND=file for headless)",
        "scopes": scopes,
    });
    println!(
        "{}",
...
async fn handle_export(unmasked: bool) -> Result<(), GwsError> {
    ...
    if !enc_path.exists() {
        return Err(GwsError::Auth(
            "No encrypted credentials found. Run 'gwsr auth login' first.".to_string(),
        ));
    }
...
fn resolve_client_credentials() -> Result<(String, String, Option<String>), GwsError> {
    ...
    let env_id = std::env::var("GWSR_CLIENT_ID").ok();
    let env_secret = std::env::var("GWSR_CLIENT_SECRET").ok();
    ...
    match crate::oauth_config::load_client_config() {
        ...
        Err(_) => Err(GwsError::Auth(format!(
            "No OAuth client configured.\n\n\
                 Either:\n  \
                   1. Run `gwsr auth setup` to configure a GCP project and OAuth client\n  \
                   2. Download client_secret.json from Google Cloud Console and save it to:\n     \
                      {}\n  \
                   3. Set env vars: GWSR_CLIENT_ID and GWSR_CLIENT_SECRET",
    ...
    if !cfg!(test) && std::io::IsTerminal::is_terminal(&std::io::stdin()) {
        ...
        if let Some(pid) = project_id {
            ...
            if !enabled_apis.is_empty() {
                ...
                if !scopes.is_empty()
                    && let Some(selected) = run_discovery_scope_picker(&scopes, services_filter)
                {
                    return selected;
                }
            }
...
fn run_discovery_scope_picker(
    ...
    use crate::setup::{PLATFORM_SCOPE, ScopeClassification};
                ...
                for (i, item) in items.iter().enumerate().skip(template_count) {
                    if item.selected {
                        ...
                        if let Some(&scope_idx) = valid_scope_indices.get(picker_idx)
                            && let Some(entry) = filtered_scopes.get(scope_idx)
                        {
                            selected.push(entry.url.to_string());
                        }
                    }
...
async fn handle_status() -> Result<(), GwsError> {
    ...
    let has_token_env = std::env::var("GWSR_TOKEN")
        .ok()
        .filter(|t| !t.is_empty())
        .is_some();
    ...
    let credential_source = if has_token_env {
        ...
        match resolve_client_credentials() {
            Ok((_, _, _)) => {
                let has_env_id = std::env::var("GWSR_CLIENT_ID").is_ok();
                let has_env_secret = std::env::var("GWSR_CLIENT_SECRET").is_ok();
    ...
    if !cfg!(test) {
        ...
        if let Some(creds_str) = creds_json_str
            && let Ok(creds) = serde_json::from_str::<serde_json::Value>(&creds_str)
        {
            ...
            if let (Some(cid), Some(csec), Some(rt)) = (client_id, client_secret, refresh_token) {
                ...
                if let Ok(http_client) = crate::client::shared_client() {
                    ...
                    if let Ok(resp) = token_resp
                        && let Ok(token_json) = resp.json::<serde_json::Value>().await
                    {
                            ...
                            token_json.get("access_token").and_then(|v| v.as_str())
                        {
                            ...
                            if let Ok(user_resp) = http_client
                                ...
                                .send()
                                .await
                                && let Ok(user_json) = user_resp.json::<serde_json::Value>().await
                                && let Some(email) = user_json.get("email").and_then(|v| v.as_str())
                            {
                            ...
                            if let Ok(info_resp) = http_client.get(&tokeninfo_url).send().await
                                && let Ok(info_json) = info_resp.json::<serde_json::Value>().await
                                && let Some(scope_str) =
...
mod tests {
    ...
    fn resolve_client_credentials_from_env_vars() {
        unsafe {
            std::env::set_var("GWSR_CLIENT_ID", "test-id");
            std::env::set_var("GWSR_CLIENT_SECRET", "test-secret");
        }
        let result = resolve_client_credentials();
        unsafe {
            std::env::remove_var("GWSR_CLIENT_ID");
            std::env::remove_var("GWSR_CLIENT_SECRET");
        }
    ...
    fn config_dir_returns_gws_subdir() {
        let path = config_dir();
        assert!(path.ends_with("gwsr"));
    }

    #[test]
    fn config_dir_primary_uses_dot_config() {
        // The primary (non-test) path should be ~/.config/gwsr.
        // We can't easily test the real function without env override,
        // but we verify the building blocks: home_dir + .config + gwsr.
        let primary = dirs::home_dir().unwrap().join(".config").join("gwsr");
        assert!(primary.ends_with(".config/gwsr") || primary.ends_with(r".config\gwsr"));
    }

    #[test]
    #[serial_test::serial]
    fn config_dir_honors_env_override() {
        // GWSR_CONFIG_DIR overrides the default location.
        let dir = tempfile::tempdir().unwrap();
        let custom = dir.path().join("custom_config");
        std::fs::create_dir_all(&custom).unwrap();

        unsafe {
            std::env::set_var("GWSR_CONFIG_DIR", custom.to_str().unwrap());
        }
        let path = config_dir();
        assert_eq!(path, custom);
        unsafe {
            std::env::remove_var("GWSR_CONFIG_DIR");
        }
    }
    ...
    fn plain_credentials_path_defaults_to_config_dir() {
        ...
        unsafe {
            std::env::remove_var("GWSR_CREDENTIALS_FILE");
        }
    ...
    fn plain_credentials_path_respects_env_var() {
        unsafe {
            std::env::set_var("GWSR_CREDENTIALS_FILE", "/tmp/test-creds.json");
        }
        let path = plain_credentials_path();
