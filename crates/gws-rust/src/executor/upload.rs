...
/// Upload content resolved to something that can be read repeatedly.
...
impl UploadData<'_> {
    pub(crate) fn len(&self) -> u64 {
        match self {
            UploadData::File { size, .. } => *size,
            UploadData::Bytes(b) => b.len() as u64,
        }
    }
        ...
        match self {
            UploadData::Bytes(b) => {
                let start = usize::try_from(start)
                    .map_err(|_| other(anyhow::anyhow!("upload offset {start} out of range")))?;
                let end = start.saturating_add(len).min(b.len());
                Ok(bytes::Bytes::copy_from_slice(&b[start.min(end)..end]))
            }
            UploadData::File { path, .. } => {
                ...
                file.seek(std::io::SeekFrom::Start(start))
                    .await
                    .map_err(|e| other(anyhow::anyhow!("failed to seek in '{path}': {e}")))?;
...
impl MultipartFrame {
    ...
    pub(crate) fn content_length(&self, data: &UploadData<'_>) -> u64 {
        self.preamble.len() as u64 + data.len() + self.postamble.len() as u64
    }

    pub(crate) fn body(&self, data: &UploadData<'_>) -> reqwest::Body {
        match data {
            UploadData::Bytes(bytes) => {
                let mut body =
                    Vec::with_capacity(self.preamble.len() + bytes.len() + self.postamble.len());
                body.extend_from_slice(self.preamble.as_bytes());
                body.extend_from_slice(bytes);
                body.extend_from_slice(self.postamble.as_bytes());
...
pub(crate) fn next_offset(headers: &reqwest::header::HeaderMap) -> Result<u64, GwsError> {
    ...
    let text = value
        .to_str()
        .map_err(|_| other(anyhow::anyhow!("non-ASCII Range header in upload status")))?;
    let last = text
        .strip_prefix("bytes=")
        .and_then(|r| r.split_once('-'))
        .and_then(|(_, end)| end.trim().parse::<u64>().ok())
        .ok_or_else(|| other(anyhow::anyhow!("unparseable upload Range header {text:?}")))?;
    Ok(last + 1)
}
...
pub(crate) async fn resumable_upload(
    ...
    loop {
        let len = usize::try_from((total - offset).min(req.chunk_size as u64))
            .map_err(|_| other(anyhow::anyhow!("chunk length overflow")))?;
...
mod tests {
    ...
    #[test]
    fn multipart_bytes_framing() {
        let data = UploadData::Bytes(b"Hello world");
    ...
    #[tokio::test]
    async fn read_range_from_file_and_bytes() {
        let dir = tempfile::tempdir().unwrap();
        ...
        let bytes = UploadData::Bytes(b"0123456789");
        assert_eq!(&bytes.read_range(8, 4).await.unwrap()[..], b"89");
    }
