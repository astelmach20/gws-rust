...
///
/// Priority:
/// 1. `GWSR_PROJECT_ID` environment variable.
...
/// 0. `GWSR_TOKEN` env var (raw access token, highest priority)
/// 1. `GWSR_CREDENTIALS_FILE` env var (plaintext JSON, can be User or Service Account)
/// 2. Encrypted credentials at `~/.config/gwsr/credentials.enc`
/// 3. Plaintext credentials at `~/.config/gwsr/credentials.json` (User only)
...
async fn load_credentials_inner(
    ...
    if let Some(path) = env_file {
        ...
        anyhow::bail!("GWSR_CREDENTIALS_FILE points to {path}, but file does not exist");
    }
    ...
    if enc_path.exists() {
        match credential_store::load_encrypted_from_path(enc_path) {
            ...
            Err(e) => {
                ...
                // Remove the stale file so the next `gwsr auth login` starts fresh,
    ...
    anyhow::bail!(
        "No credentials found. Run `gwsr auth setup` to configure, \
         `gwsr auth login` to authenticate, or set GWSR_CREDENTIALS_FILE.\n\
...
mod tests {
    ...
    async fn test_get_token_from_env_var() {
        let _token_guard = EnvVarGuard::set("GWSR_TOKEN", "my-test-token");
    ...
    async fn test_scoped_token_provider_uses_get_token() {
        let _token_guard = EnvVarGuard::set("GWSR_TOKEN", "provider-token");
    ...
    async fn test_get_token_env_var_empty_falls_through() {
        ...
        let _home_guard = EnvVarGuard::set("HOME", tmp.path());
        let _adc_guard = EnvVarGuard::remove("GOOGLE_APPLICATION_CREDENTIALS");
        let _token_guard = EnvVarGuard::set("GWSR_TOKEN", "");
    ...
    fn test_get_quota_project_priority_env_var() {
        let _env_guard = EnvVarGuard::set("GWSR_PROJECT_ID", "priority-env");
        let _adc_guard = EnvVarGuard::remove("GOOGLE_APPLICATION_CREDENTIALS");
        let _config_guard = EnvVarGuard::remove("GWSR_CONFIG_DIR");
    ...
    fn test_get_quota_project_priority_config() {
        let tmp = tempfile::tempdir().unwrap();
        let _config_guard = EnvVarGuard::set("GWSR_CONFIG_DIR", tmp.path().to_str().unwrap());
        let _env_guard = EnvVarGuard::remove("GWSR_PROJECT_ID");
        let _adc_guard = EnvVarGuard::remove("GOOGLE_APPLICATION_CREDENTIALS");
        let _home_guard = EnvVarGuard::set("HOME", "/missing/home");
