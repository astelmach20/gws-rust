...
///
/// Controlled by `GWSR_KEYRING_BACKEND`:
...
impl KeyringBackend {
    fn from_env() -> Self {
        let raw = std::env::var("GWSR_KEYRING_BACKEND").unwrap_or_default();
        ...
        match lower.as_str() {
            ...
            other => {
                ...
                eprintln!(
                    "Warning: unknown GWSR_KEYRING_BACKEND=\"{other}\", \
    ...
    if backend == KeyringBackend::Keyring {
        #[cfg(any(target_os = "macos", target_os = "windows"))]
        {
            match provider.get_password() {
                ...
                Err(e) => {
                    anyhow::bail!(
                        "OS keyring failed: {}. Set GWSR_KEYRING_BACKEND=file to use file storage.",
                        sanitize_for_terminal(&e.to_string())
                    );
                }
            }
...
/// Backend selection is controlled by `GWSR_KEYRING_BACKEND`.
fn get_or_create_key() -> anyhow::Result<[u8; 32]> {
    ...
    let provider = OsKeyring::new("gwsr-cli", &username);
    ...
    let plaintext = cipher.decrypt(nonce, &data[12..]).map_err(|_| {
        anyhow::anyhow!(
            "Decryption failed. Credentials may have been created on a different machine. \
                 Run `gwsr auth logout` and `gwsr auth login` to re-authenticate."
        )
    })?;
