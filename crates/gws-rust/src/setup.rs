...
/// Build the clap Command for `gwsr auth setup`.
    ...
    if ctx.interactive {
        ...
        let mut items: Vec<SelectItem> = vec![
            SelectItem {
                ...
                description: "Create a new GCP project for gwsr".to_string(),
                selected: false,
                is_fixed: false,
                is_template: false,
                template_selects: vec![],
...
async fn stage_enable_apis(ctx: &mut SetupContext) -> Result<SetupStage, GwsError> {
    ...
    if ctx.dry_run {
        ...
        let output = json!({
            "status": "dry_run",
            "message": "No changes were made. Run `gwsr auth login` to authenticate.",
            "account": ctx.account,
...
///
/// Returned as the error message when `gwsr auth setup` cannot prompt interactively,
...
fn manual_oauth_instructions(project_id: &str) -> String {
    ...
    format!(
        concat!(
            ...
            "   {consent_url}\n",
            "   → User Type: External\n",
            "   → App name: gwsr CLI (or your preferred name)\n",
...
pub async fn run_setup(args: &[String]) -> Result<(), GwsError> {
    ...
    let message = if run_login {
        ...
        "Setup complete! Run `gwsr auth login` to authenticate."
    };
