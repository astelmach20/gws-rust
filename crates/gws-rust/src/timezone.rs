...
/// Cache filename stored in the gwsr config directory.
...
async fn fetch_account_timezone(client: &reqwest::Client, token: &str) -> Result<Tz, GwsError> {
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| GwsError::from(anyhow::anyhow!("Failed to fetch account timezone: {e}")))?;
    ...
    let json: serde_json::Value = resp
        .json()
        .await
        .map_err(|e| GwsError::from(anyhow::anyhow!("Failed to parse timezone response: {e}")))?;
...
pub fn start_of_today(tz: Tz) -> Result<chrono::DateTime<Tz>, crate::error::GwsError> {
    ...
    tz.from_local_datetime(&today_start)
        .earliest()
        .ok_or_else(|| {
            crate::error::GwsError::from(anyhow::anyhow!(
