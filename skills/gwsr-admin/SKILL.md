---
name: gwsr-admin
description: "Google Workspace Admin SDK: Manage users, groups, org units, devices, and roles (Admin SDK Directory)."
metadata:
  version: 0.23.1
  openclaw:
...
```bash
gwsr admin <resource> <method> [flags]
```
...
| Command | Description |
|---------|-------------|
| [`+user-create`](../gwsr-admin-user-create/SKILL.md) | Create a user account |
| [`+user-suspend`](../gwsr-admin-user-suspend/SKILL.md) | Suspend (or with --unsuspend, restore) a user |
| [`+group-add-member`](../gwsr-admin-group-add-member/SKILL.md) | Add a user or group to a group |
...
gwsr schema admin.<resource>.<method>
```
